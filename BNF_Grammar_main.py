...
def parse(self):
    ...
    if (token == "float"): exp(self)
        ...
        if (inToken[1] == ":"):
            ...
            self.finalBox.insert(INSERT, "\nparse tree building was successful" + '\n')
        else:
            print("Expected separator in if statement!")
            self.finalBox.insert(INSERT, "Expected separator in if statement!" + '\n')
            return
...
class lexerGUI:
    ...
    def __init__(self, root):
        self.currentLine = 0
        self.master = root
        self.master.title("HW7 parser project")

        self.inputLabel = Label(self.master, text="Source Code Input:")
        self.inputLabel.grid(row=0, column=1)
        self.inputBox = Text(self.master, height=35, width=50)
        self.inputBox.grid(row=1, column=1, padx=20, columnspan=2)

        self.outputLabel = Label(self.master, text="Tokens:")
        self.outputLabel.grid(row=0, column=3)
        self.outputBox = Text(self.master, height=35, width=50)
        self.outputBox.grid(row=1, column=3, padx=20, columnspan=2)

        self.finalLabel = Label(self.master, text="Parse Tree:")
        self.finalLabel.grid(row=0, column=5)
        self.finalBox = Text(self.master, height=35, width=50)
        self.finalBox.grid(row=1, column=5, padx=20, columnspan=2)

        self.lineNumberDisplay = Entry(self.master, width=3)
...
def cutOneLineTokens(line):
    ...
    while (line != ""):
        line = line.lstrip()
        test1 = re.match(r'\b(if|else|int|float|print)\b', line)
        if (test1 != None):
        ...
        test4 = re.match(r'([A-Z]+|[a-z]+)([A-Z]+|[a-z]+)*[0-9]*', line)
        if (test4 != None):
...
def removeFromStr(fullLine, str):
    length = len(str)
